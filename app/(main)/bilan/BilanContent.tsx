    ...
    <Shell>
      ...
      >
        {isDesktop ? (
          ...
          <div className="flex mb-4">
            <MiniKpi label="Revenus" value={`${formatEur(totR)}`} color="#22c55e" />
            <div className="w-px bg-white/[0.06]" />
            <MiniKpi label="Dépenses" value={`${formatEur(totD)}`} color="#ef4444" />
            <div className="w-px bg-white/[0.06]" />
            <MiniKpi label="Épargne" value={`${formatEur(totEp)}`} color="#f97316" />
          </div>
        )}
      </motion.div>
      ...
      {bars.length > 0 ? (
        <motion.div
          ...
          className="mb-4 bg-[#0d0d1a] border border-white/[0.06] rounded-xl p-4"
        >
        ...
        <div className="text-center py-12 text-white/28 text-[11px]">
      ...
      {/* ── Global taux from current period stats ── */}
      {!isDesktop && (
        <div className="mt-4 bg-[#0d0d1a] border border-white/[0.06] rounded-xl px-3 py-2.5">
          <div className="flex items-center justify-between mb-1.5">
            <p className="text-[9px] text-white/28 uppercase tracking-[0.14em] font-semibold">Taux moyen période</p>
            <span className="text-[12px] font-bold" style={{ color: savingsColor(tauxMoy) }}>{tauxMoy}%</span>
          </div>
          <BarComp pct={Math.max(0, tauxMoy)} color={savingsColor(tauxMoy)} height={4} />
        </div>
      )}
    </Shell>
  );
...
function MiniKpi({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div className="flex-1 px-3 py-2">
      <p className="text-[9px] text-white/28 uppercase tracking-[0.12em] font-semibold">{label}</p>
      <p className="text-[15px] font-[900] tracking-tight mt-1 leading-none" style={{ color }}>{value} €</p>
    </div>
  );
}
